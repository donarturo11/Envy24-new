...
int card_init(struct CardData *card)
{
    ...
    // MT 32 Routing Control Register for SPDOUT
    // 0101 = 0x8005 to route mixer and SPDIN as input to SPDOUT
    dev->ioWrite16(0x32, 0x0101, card->mtbase);
    
    
    // loopback psdin[0] to psdout[0] -- removed from original code
	...
	IOLog("Envy24: Turned on monitoring. Mixer output  1 (L) 2 (R) and SPDOUT.\n");
    IOLog("For a stereo output, DMA 1,3,5,7 are routed to Mixer L, PCM 2,4,6,8 are routed to Mixer R\n");
    IOLog("Use Thru toggle of Audio & MIDI Configuration panel to enable hardware input monitoring for each channel. Default is off\n");
